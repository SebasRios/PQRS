...
// Model for Pqrs entity
    models.Pqrs = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/PQRS/webresources/com.airline.modelo.pqrs/",
        idAttribute: 'codigo',
        defaults: {
            descripcion: "",
            tipo: "",
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('codigo');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/PQRS/webresources/com.airline.modelo.pqrs/';
            }
    ...
    // Collection class for Pqrs entities
    models.PqrsCollection = Backbone.Collection.extend({
        model: models.Pqrs,
        url: "http://localhost:8080/PQRS/webresources/com.airline.modelo.pqrs/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.PqrsCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-pqrs-list-itemis template identifier for item
                        templateName: '#tpl-pqrs-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.pqrs = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.pqrs,
                    // tpl-pqrs-details is template identifier for chosen model element
                    templateName: '#tpl-pqrs-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Pqrs();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-pqrs-details is a template identifier for chosen model element
                templateName: '#tpl-pqrs-details',
        ...
        getData: function () {
            return {
                ...
                codigoAeropuerto: $('#codigoAeropuerto').val(),
                numeroDocCliente: $('#numeroDocCliente').val()
            };
        }
    });
    new AppRouter();
